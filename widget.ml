...
let list_layout (pair: widget -> widget -> widget) (ws: widget list) : widget =
  ...
  (* fold_right (fun num acc -> pair num acc) space (0, 0) ws *)
...
(** A checkbox is a controller for a boolean value associated with a widget.
   ...
   We will use this value_controller as part of the checkbox implementation, 
   and you are free to use it (if needed) for whatever widget you create in
   Task 6. *)
...
let make_controller (v: 'a) : 'a value_controller =
  ...
  let controller_listeners = {contents : ('a -> unit) list = []} in
  {add_change_listener = 
  (fun x -> (controller_listeners.contents <- 
    controller_listeners.contents @ [x]));
  get_value = (fun () -> value.contents);
  change_value = (fun nv -> (value.contents <- nv; List.iter (fun x -> x nv) 
    controller_listeners.contents))
  }
...
let checkbox (init: bool) (s: string) : widget * bool value_controller =
  ...
  let w = { 
    ...
    handle = (fun (g: Gctx.gctx) (e: Gctx.event) -> 
      ...
      if event_within g e (20, 20) && t = Gctx.MouseDown 
      then vc.change_value (not (vc.get_value ())));
    size = fun () -> (20, 20)
    } in
  let labeled = hpair w l in
  (labeled, vc)
...
let slider (init: int) (s: string) : widget * int value_controller =
  ...
  let w = { 
    repaint = (fun (g: Gctx.gctx) ->
      Gctx.draw_line g (0, 0) (200, 0);
      let c = Gctx.with_color g Gctx.black in
      Gctx.fill_rect c (0, 0) (vc.get_value(), 8));
    handle = (fun (g: Gctx.gctx) (e: Gctx.event) -> 
      let t = Gctx.event_type e in
      if (event_within g e (200, 8)) 
      && (t = Gctx.MouseDrag || t = Gctx.MouseDown) 
      then let (x, _) = Gctx.event_pos e g in
      vc.change_value x);
    size = fun () -> (200, 8)
    } in
  let labeled =vlist [l; w] in
  (labeled, vc)